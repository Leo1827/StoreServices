...
import { sendVerificationLink } from '../services/RegisterServicesVerifyEmail'
import { closeAllModals, openModal } from '../store/slices/modal/modalSlice'

const CardRegisterVerify = (): JSX.Element => {
    const { email } = useAppSelector((state) => state.auth)
    const dispatch = useAppDispatch()

    const [isDisabled, setIsDisabled] = useState(false) // se controla si el boton esta deshabilitado o no
    const [countdown, setCountdown] = useState(0) // almacena el tiempo restante en segundos para habilitar el boton

    useEffect(() => {
        // se crea un intervalo de tiempo que se ejecuta cada segundo cuando el boton esta deshabilitado
    ...
    const handleSendEmailVerify = (): void => {
        setIsDisabled(true)
        sendVerificationLink(email)
            .then((resp: any) => {
                if (resp.status === 200) {
                    const payloadModal = {
                        modal: 'notification-msg',
                        data: [],
                    }
            ...
            .finally(() => {
                setTimeout(() => {
                    setIsDisabled(false)
                }, 120000) // Deshabilita el botón por 2 minutos (120000 milisegundos)
            })
    }
    ...
    return (
        <div className="cardRegisterVerify container p-0">
            <div className="verify-valid container text-center">
                <div className="row verify-spacing-top">
                    ...
                    <div className="border-web container">
                        <div className="mt-1 container w-100 mb-3">
                            <div className="mesagge-card p-1">
                                <span
                                    className="pb-1 verify-text"
                                    style={{ fontSize: '.9em' }}
                                >
                    ...
                    <div className="col-md-12 mt-2 mb-5 pb-3 chronometer-phone">
                        <span className="row d-flex justify-content-center my-3 chronometer-verify">
...
export default CardRegisterVerify
